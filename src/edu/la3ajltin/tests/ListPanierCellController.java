...
public class ListPanierCellController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Stream<Produit> sp = Session.getPanier().stream();
    ...
    private void ProceedWithChekout(ActionEvent event) throws Exception {
        ...
        CommandeProduitsService cps = new CommandeProduitsService();
        ProduitService ps = new ProduitService();
        System.out.println(Session.getCurrentSession());
        Commande c = new Commande(Session.getCurrentSession(), Float.valueOf(prixtot.getText()), Date.valueOf(java.time.LocalDate.now()));

        cs.ajouterProduit(c);
        for (int i = 0; i < getPanier().size(); i++) {
            Produit produit = getPanier().get(i);
            System.out.println(((produit.getQuantite() * 20) / 100));
            if (ps.getQuantiteAV(produit) == ((produit.getQuantite() * 20) / 100)) {
                sendMail(produit);
            }
            System.out.println(produit.getQuantite() - ps.getQuantiteAV(produit));
            cs.updateQuantite(produit.getQuantite() - ps.getQuantiteAV(produit), ps.getQuantiteAV(produit) - produit.getQteProduitPanier(), produit.getId_prod());
            cps.ajouter(produit.getId_prod(), produit.getQteProduitPanier());
        }
        sendInvoice(c);
        try {
            ...
            trayIcon.displayMessage("Commande, ", "Ajoutée avec succès !!", TrayIcon.MessageType.INFO);
            // Error:
